$.extend({
	...
	dispatchEvent: function(e)
	{
		...
		if (e.type == 'click') {
			...
			// Check if we clicked on link that should send ajax request
				...
				if (link_obj.hasClass('cm-history') && $.ceDialog('inside_dialog', {elm: link_obj}) == false) {
					...
					var render_current = link_obj.hasClass('cm-ajax-render-current');
					...
					if (render_current) {
						data.render_current = render_current;
		...
		// Dispatch submit event
            ...
            if (char_code == 27) {
				$.ceDialog('get_last').ceDialog('close');
				var id = $('.product-notification-container:visible').attr('id').str_replace('notification_', '');
				if (id) {
...
(function($){
	var methods = {
		...
		_get_container_height: function(container) {
			...
			var buttons_elm = container.find('.buttons-container:last');
			if (buttons_elm.length) {
...
(function($){
	$.ceScrollerMethods = {
		...
		init_callback: function(carousel, state) {
			...
			var container_width = carousel.buttonNext.outerWidth() * 2 + carousel.options.clip_width;
			...
			// fix prev/next links position
			var button_height = carousel.buttonNext.outerHeight();
			var scroll_height = (carousel.options.item_height + button_height)/2;
			carousel.buttonNext.css('margin-top', '-' + scroll_height + 'px');
			carousel.buttonPrev.css('margin-top', '-' + scroll_height + 'px');
