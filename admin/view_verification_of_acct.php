...
<table>
<tr><th colspan="12">Verification Of Company Accounts</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
<th>Office Location</th>
<th>Companies Verified</th>
<th>Variance Est. in Cash</th>
<th>Amount Paid</th>
<th>Outstanding</th>
<th>Added by</th>
<th>Month</th>
...
if($access_right == 4){
	$query_report_view = "select * from ver_comp_acct $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from ver_comp_acct where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from ver_comp_acct where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$comp_verd = mysqli_result($result_report_view,  $i,  "comp_verd");
	$var_est_cash = mysqli_result($result_report_view,  $i,  "var_est_cash");
	$amount = mysqli_result($result_report_view,  $i,  "amount");
	
	$balance = $var_est_cash - $amount;
...
<td bgcolor="'.$bg_color.'"><a href="view_comp_verd.php?id='.$rep_id.'">'.$comp_verd.'</a></td>
<td bgcolor="'.$bg_color.'">'.$var_est_cash.'</td>
<td bgcolor="'.$bg_color.'">'.$amount.'</td>
<td bgcolor="'.$bg_color.'">'.$balance.'</td>
...
$over_total = mysqli_query( $connect, "select sum(comp_verd), sum(var_est_cash), sum(amount) from ver_comp_acct " .$opt." ")or
...
<td bgcolor="#00FFFF">'.($result_sum[1] - $result_sum[2]).'</td>
<td></td>
</tr>';
