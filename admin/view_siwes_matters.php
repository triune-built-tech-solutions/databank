...
<table>
<tr><th colspan="16">Siwes Matters</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
...
if($access_right == 4){
	$query_report_view = "select * from siwes_matters $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from siwes_matters where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$student_eligable = mysqli_result($result_report_view,  $i,  "student_eligable");
...
<td bgcolor="'.$bg_color.'">'.$student_eligable.'</td>
...
$over_total = mysqli_query( $connect, "select sum(part_inst), sum(orien_d_month), sum(zonal_meet), sum(student_placed), sum(student_eligable), sum(srudent_paid), sum(amount_paid), sum(supv_allowance), sum(students_inv) from siwes_matters " .$opt." ")or
