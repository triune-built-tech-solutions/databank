...
<table>
<tr><th colspan="13">Reimbursement Reports</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
...
if($access_right == 4){
	$query_report_view = "select * from reimbursement $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from reimbursement where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$claims_files = mysqli_result($result_report_view,  $i,  "claims_files");
...
<td bgcolor="'.$bg_color.'">'.$claims_files .'</td>
...
$over_total = mysqli_query( $connect, "select sum(claims_rec), sum(claims_proc), sum(claims_paid), sum(claims_files), sum(amount_paid) from reimbursement " .$opt." ")or
