...
if(isset($opt)){
	$query_part = "select * from comp_outstanding $opt order by name limit 80";
} else {
	$query_part = "select * from comp_outstanding order by name limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_part); $i++){
	...
	$amount_due = mysqli_result($result_part,  $i,  "amount_due");
	$amount_paid = mysqli_result($result_part,  $i,  "amount_paid");
	$payment_year = mysqli_result($result_part,  $i,  "payment_year");
...
<table>
<tr><th colspan="11">Outstanding Course Fee Company Details</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
...
<td bgcolor="'.$bg_color.'">'.$amount_due.'</td>
<td bgcolor="'.$bg_color.'">'.$amount_paid.'</td>
<td bgcolor="'.$bg_color.'">'.$payment_year.'</td>
