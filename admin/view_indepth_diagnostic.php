...
<table>
<tr><th colspan="12">New Training Programme Proposal</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
<th>Office Location</th>
<th>Target</th>
<th>Studies Carried Out</th>
<th>Intervention Developed</th>
...
if($access_right == 4){
	$query_report_view = "select * from msme $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from msme where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from msme where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$studies_c_o = mysqli_result($result_report_view,  $i,  "studies_c_out");
	$interv_dev = mysqli_result($result_report_view,  $i,  "interv_dev");
	$interv_imp = mysqli_result($result_report_view,  $i,  "interv_imp");
...
<td bgcolor="'.$bg_color.'">'.$studies_c_o.'</td>
<td bgcolor="'.$bg_color.'">'.$interv_dev.'</td>
<td bgcolor="'.$bg_color.'">'.$interv_imp.'</td>
