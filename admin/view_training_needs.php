...
<table>
<tr><th colspan="11">Training Needs Assessment</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
...
if($access_right == 4){
	$query_report_view = "select * from training_needs $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from training_needs where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
...
<td bgcolor="'.$bg_color.'">'.$train_int_dev.'</td>
<td bgcolor="'.$bg_color.'">'.$train_int_imp.'</td>
...
$over_total = mysqli_query( $connect, "select sum(target), sum(surveys), sum(train_inter_dev), sum(train_inter_imp) from training_needs $opt");
