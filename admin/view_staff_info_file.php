...
<table>
<tr><th colspan="12">Minutes of Meeting</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
...
if($access_right == 4){
	$query_report_view = "select * from staff_info_file $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from staff_info_file where off_l = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from staff_info_file where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
<td bgcolor="'.$bg_color.'">'.$heading.'</td>
...
<td bgcolor="'.$bg_color.'"><a href="uploaded/'.$filename.'">Download</a></td>
</tr>';
}
?>
</table>
