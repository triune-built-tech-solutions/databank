...
<table>
<tr><th colspan="12">Training Contribution</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
<th>Office Location</th>
<th>Target</th>
<th>Amount Collected</th>
<th>Total Amount Collected to Date</th>
<th>Added by</th>
<th>Month</th>
<th>Year </th>
<th>Date Added</th>
...
if($access_right == 4){
	$query_report_view = "select * from train_cont $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from train_cont where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from train_cont where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$total_t_date = mysqli_result($result_report_view,  $i,  "total_t_date");
	...
	$new_query = "select sum(amount_coll) from train_cont where office_location = $office_location and year = '$year'";
	$new_result = mysqli_query( $connect, $new_query);
	while($row_sum_coll = mysqli_fetch_array($new_result)){
		$sum_coll_tdate = $row_sum_coll['0'];
	}
	
	$sum_amount = 0;
	$sum_amount += $amount_coll;
...
<td bgcolor="'.$bg_color.'">'.$sum_coll_tdate.'</td>
...
$over_total = mysqli_query( $connect, "select sum(target), sum(amount_coll) from train_cont " .$opt." ")or
