...
<table>
<tr><th colspan="10">Industrial Based Research Proposal</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
<th>Office Location</th>
<th>Target</th>
<th>Surveys Carried Out</th>
<th>Training Interview Imp.</th>
<th>Added by</th>
<th>Month</th>
<th>Year </th>
<th>Date Added</th>
...
if($access_right == 4){
	$query_report_view = "select * from industrial_based $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from industrial_based where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from industrial_based where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$surveys = mysqli_result($result_report_view,  $i,  "surveys");
	$training = mysqli_result($result_report_view,  $i,  "train_interv_imp");
...
<td bgcolor="'.$bg_color.'">'.$surveys.'</td>
