...
<table>
<tr><th colspan="11">Outstanding Course Fee from Previous Years</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
...
if($access_right == 4){
	$query_report_view = "select * from outs_c_f_prev_y $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from outs_c_f_prev_y where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from outs_c_f_prev_y where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$gross_amount = mysqli_result($result_report_view,  $i,  "gross_amount");
	$amount_coll = mysqli_result($result_report_view,  $i,  "amount_coll");
	
	$balance = $gross_amount - $amount_coll;
...
<td bgcolor="'.$bg_color.'">'.$gross_amount.'</td>
...
$over_total = mysqli_query( $connect, "select sum(gross_amount), sum(amount_coll) from outs_c_f_prev_y " .$opt." ")or
...
<td bgcolor="#00FFFF">'.($result_sum[0] - $result_sum[1]).'</td>
</tr>';
