...
if($access_right == 4){
	$query_report_view = "select * from rev_fr_course $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from rev_fr_course where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$exp_rev = mysqli_result($result_report_view,  $i,  "exp_rev");
	$prog_imp = mysqli_result($result_report_view,  $i,  "prog_imp");
	$amt_budg = mysqli_result($result_report_view,  $i,  "amt_budg");
	$amt_expen = mysqli_result($result_report_view,  $i,  "amt_expen");
...
<td bgcolor="'.$bg_color.'">'.$exp_rev.'</td>
<td bgcolor="'.$bg_color.'">'.$amount_coll.'</td>
<td bgcolor="'.$bg_color.'">'.$amount_outs.'</td>
<td bgcolor="'.$bg_color.'">'.$amt_budg.'</td>
<td bgcolor="'.$bg_color.'">'.$amt_expen.'</td>
...
$over_total = mysqli_query( $connect, "select sum(amount_coll), sum(exp_rev), sum(amt_budg), sum(net_rev), sum(amount_outs), sum(prog_imp), sum(amt_expen) from rev_fr_course $opt");
