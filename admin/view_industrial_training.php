	...
	<div id="content"><!-- open content -->
		...
		<div>
			<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="report_query" class="form-group form-inline d-flex justify-content-center">
				...
				<select class="form-control ml-3" name="office_loc" id="office_loc">
					<option value=" "></option>
				</select>&nbsp;&nbsp;&nbsp; <button class="btn btn-success" type="submit" name="begin_query" value="Begin Query">Query</button>
			</form>
			<br/>
			...
			<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="period_query" class="form-group form-inline d-flex justify-content-center">
				Periodic Query : <select class="form-control ml-3 mr-3" name="month_f" id="month_f">
					...
					?>
				</select> &nbsp;&nbsp;||&nbsp;&nbsp; <select class="form-control ml-3" name="month_t" id="month_t">
					...
					?>
				</select>&nbsp;&nbsp;&nbsp;&nbsp; <select class="form-control ml-3" name="p_year">
					<option value=" ">-Year-</option>
					<?php
					$query_eyear = "Select * from eyear";
					$result_eyear = mysqli_query($connect, $query_eyear);
					...
					?>
				</select>&nbsp;&nbsp; <button class="btn btn-primary" type="submit" name="begin_period" value="Query">Begin Query</button>
			</form>
		</div>
		<br/>
		...
		<div class="card shadow">
			<div class="card-header">
				<h4>Apprenticeship Standardization and Evaluation</h4>
			</div>
			<div class="card-body">
				<table class="table-hover table table-responsive">
					<thead>
						<tr>
							...
							<th>Harmonized</th>
							<th>Monitored</th>
							<th>Provisional Apprenticeship</th>
							<th>Full Apprenticeship</th>
							<th>Added by</th>
							<th>Month</th>
							<th>Year</th>
							<th>Date Added</th>
			...
			alert("Report for specified office and date already exist");
					...
					if ($access_right == 4) {
						$query_report_view = "select * from industrial_training $opt order by year desc, month desc limit 80";
					} else if ($access_right == 3 || $access_right == 1) {
						$query_report_view = "select * from industrial_training where office_location = '" . $office_location . "' order by year desc, month desc limit 80";
					} else {
					...
					for ($i = 0; $i < mysqli_num_rows($result_report_view); $i++) {
						...
						$harmonized = mysqli_result($result_report_view, $i, "harmonized");
						$prov_apprentice = mysqli_result($result_report_view, $i, "prov_apprentice");
						$full_apprentice = mysqli_result($result_report_view, $i, "full_apprentice");
...
<td >' . $harmonized . '</td>
<td >' . $monitored . '</td>
<td >' . $prov_apprentice . '</td>
<td >' . $full_apprentice . '</td>
<td >' . $added_by . '</td>
<td >' . $month_row[1] . '</td>
<td >' . $year . '</td>
<td >' . $auto_date . '</td>
						...
						$over_total = mysqli_query($connect, "select sum(prom_visits), sum(appraisal), sum(install_harmonized), sum(harmonized), sum(monitored), sum(prov_apprentice), sum(full_apprentice) from industrial_training " . $opt . " ") or
