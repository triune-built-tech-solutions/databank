...
<div id="content"><!-- open content -->
	...
	<div class="">
		<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="report_query"
		      class="d-flex form-inline form-group justify-content-center">
			...
			<select name="year" class="form-control ml-3">
			...
			<select name="office_type" id="office_t" class="form-control ml-3">
			...
			<select name="office_loc" id="office_loc" class="form-control ml-3">
				<option value=" "></option>
			</select>&nbsp;&nbsp;&nbsp;
			<button class="btn btn-success ml-3" type="submit" name="begin_query" value="Begin Query">Begin Query
			</button>
		</form>
		<br/>
		...
		<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="period_query"
		      class="d-flex form-inline form-group justify-content-center">
			<label for="" class="mr-3">Periodic Query :</label>
			<select name="month_f" id="month_f" class="form-control">
			...
			<select name="month_t" id="month_t" class="form-control ml-3">
			...
			<select name="p_year" class="form-control ml-3">
				<option value=" ">-Year-</option>
				<?php
				$query_eyear = "Select * from eyear";
				$result_eyear = mysqli_query($connect, $query_eyear);
	...
	<div class="card shadow mt-5">
		...
		<div class="card-body">
			...
			alert("Report for specified office and date already exist");
				...
				if ($access_right == 4) {
					$query_report_view = "select * from scheduled_training $opt order by year desc, month desc limit 80";
				} else if ($access_right == 3 || $access_right == 1) {
					$query_report_view = "select * from scheduled_training where office_location = '" . $office_location . "' order by year desc, month desc limit 80";
				} else {
...
<td >' . $training . '</td>
<td >' . $implemented . '</td>
<td ><a href="add_scheduled_part.php?id=' . $rep_id . '">' . $participants . '</a></td>
<td >' . $organization . '</td>
<td >' . $rev_f_c . '</td>
<td >' . $duration . '</td>
<td >' . $added_by . '</td>
...
<td >' . $year . '</td>
<td >' . $auto_date . '</td>
<td ><a href="add_scheduled_part.php?id=' . $rep_id . '">Add Participants</a></td>
</tr>';

					$over_total = mysqli_query($connect, "select sum(training), sum(implemented), sum(participants), sum(organizations), sum(revenue_f_c) from scheduled_training $opt");
