...
<table>
<tr><th colspan="16">Course Approvals</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
<th>Office Location</th>
<th>Comp. Requesting</th>
<th>Courses Approved (Local)</th>
<th>Courses Approved (Oversea)</th>
<th>Courses Rejected (Local)</th>
<th>Courses Rejected (Oversea)</th>
<th>Trainees for Course (Local)</th>
<th>Trainees for Course (Oversea)</th>
<th>Approved Progs.</th>
<th>Comp. Involved</th>
<th>Added by</th>
<th>Month</th>
<th>Year </th>
...
if($access_right == 4){
	$query_report_view = "select * from course_approvals $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from course_approvals where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from course_approvals where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$comp_request = mysqli_result($result_report_view,  $i,  "comp_request");
	$courses_appr = mysqli_result($result_report_view,  $i,  "courses_appr");
	$courses_appr_over = mysqli_result($result_report_view,  $i,  "courses_app_over");
	$courses_rej = mysqli_result($result_report_view,  $i,  "courses_rej");
	$courses_rej_over = mysqli_result($result_report_view,  $i,  "courses_rej_over");
	$trainees = mysqli_result($result_report_view,  $i,  "trainees");
	$appr_prog = mysqli_result($result_report_view,  $i,  "appr_prog");
	$trainees_f_c = mysqli_result($result_report_view,  $i,  "trainees_f_c");
	$comp_invl = mysqli_result($result_report_view,  $i,  "comp_invl");
...
<td bgcolor="'.$bg_color.'">'.$comp_request.'</td>
<td bgcolor="'.$bg_color.'">'.$courses_appr.'</td>
<td bgcolor="'.$bg_color.'">'.$courses_appr_over.'</td>
<td bgcolor="'.$bg_color.'">'.$courses_rej.'</td>
<td bgcolor="'.$bg_color.'">'.$courses_rej_over.'</td>
<td bgcolor="'.$bg_color.'">'.$trainees.'</td>
<td bgcolor="'.$bg_color.'">'.$trainees_f_c.'</td>
<td bgcolor="'.$bg_color.'">'.$appr_prog.'</td>
<td bgcolor="'.$bg_color.'">'.$comp_invl.'</td>
...
$over_total = mysqli_query( $connect, "select sum(comp_request), sum(courses_appr), sum(courses_app_over), sum(courses_rej), sum(courses_rej_over), sum(trainees), sum(trainees_f_c), sum(appr_prog), sum(comp_invl) from course_approvals " .$opt." ")or
...
<td bgcolor="#00FFFF">'.$result_sum[5].'</td>
<td bgcolor="#00FFFF">'.$result_sum[6].'</td>
<td bgcolor="#00FFFF">'.$result_sum[7].'</td>
<td bgcolor="#00FFFF">'.$result_sum[8].'</td>
</tr>';
