...
if(isset($_REQUEST['delete'])){
	...
	for($j=0;$j<count($ids);$j++){
		$new_id = $ids[$j];
		$w = "select * from retired_staff where id= ".$new_id;
		$result = mysqli_query( $connect, $w) or
		die ("Error Selecting from table ".mysqli_error($GLOBALS["___mysqli_ston"]));
		
		$row = mysqli_fetch_array($result);
		
		$x = "INSERT INTO nominal value($row[0], '$row[1]', '$row[2]', '$row[3]', '$row[4]', '$row[5]', '$row[6]', '$row[7]', '$row[8]', '$row[9]', '$row[10]')";
	...
	for($i=0;$i<count($ids);$i++){
		$news_id = $ids[$i];
		$q = "delete from retired_staff where id= ".$news_id;
