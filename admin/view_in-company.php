...
<table>
<tr><th colspan="12">In-Company Safety Reports</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
<th>Office Location</th>
<th>Target</th>
<th>Survey Conducted</th>
<th>Prog. Implemented</th>
<th>Participants</th>
<th>Organization</th>
<th>Added by</th>
<th>Month</th>
...
if($access_right == 4){
	$query_report_view = "select * from incompany $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from incompany where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from incompany where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$senior_staff = mysqli_result($result_report_view,  $i,  "survey_target");
	$survey = mysqli_result($result_report_view,  $i,  "survey_conducted");
	$prog_impl = mysqli_result($result_report_view,  $i,  "prog_implemented");
	$particpants = mysqli_result($result_report_view,  $i,  "participants");
	$organization = mysqli_result($result_report_view,  $i,  "organization");
...
<td bgcolor="'.$bg_color.'">'.$survey .'</td>
<td bgcolor="'.$bg_color.'">'.$prog_impl .'</td>
<td bgcolor="'.$bg_color.'"><a href="view_in_participant.php?id='.$rep_id.'">'.$particpants .'</a></td>
<td bgcolor="'.$bg_color.'"><a href="view_in_organization.php?id='.$rep_id.'">'.$organization .'</a></td>
...
$over_total = mysqli_query( $connect, "select sum(survey_target), sum(survey_conducted), sum(prog_implemented), sum(participants), sum(organization) from incompany " .$opt." ")or
