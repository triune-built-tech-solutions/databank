...
<table>
<tr><th colspan="12">Employers Statistics</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
<th>Office Location</th>
<th>Reg. to Date</th>
<th>Dis. During The Month</th>
<th>Reg. During The Month</th>
<th>Cont. Up-to-Date</th>
<th>Defaulting</th>
<th>Added by</th>
<th>Month</th>
<th>Year </th>
...
if($access_right == 4){
	$query_report_view = "select * from emp_stat $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from emp_stat where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from emp_stat where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
<td bgcolor="'.$bg_color.'">'.$reg_t_date.'</td>
<td bgcolor="'.$bg_color.'">'.$dis_d_month.'</td>
<td bgcolor="'.$bg_color.'">'.$reg_d_month.'</td>
<td bgcolor="'.$bg_color.'">'.$con_t_date.'</td>
<td bgcolor="'.$bg_color.'"><a href="view_emp_stat_organization.php?id='.$rep_id.'">'.$defaulting.'</a></td>
...
$over_total = mysqli_query( $connect, "select sum(reg_t_date), sum(dis_d_month), sum(reg_d_month), sum(con_t_date), sum(defaulting) from emp_stat " .$opt." ")or
