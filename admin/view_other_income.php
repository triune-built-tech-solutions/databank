...
<table>
<tr><th colspan="12">Other Income Generated</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
<th>Office Location</th>
<th>Target</th>
<th>Amount Generated</th>
<th>Total Amount Gen.</th>
<th>Variance</th>
<th>Added by</th>
<th>Month</th>
<th>Year </th>
...
if($access_right == 4){
	$query_report_view = "select * from other_inc $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from other_inc where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from other_inc where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$amount_gen = mysqli_result($result_report_view,  $i,  "amount_gen");
	$total_amt_gen = mysqli_result($result_report_view,  $i,  "total_amt_gen");
	
	$balance = $total_amt_gen - $amount_gen;
...
<td bgcolor="'.$bg_color.'">'.$amount_gen.'</td>
<td bgcolor="'.$bg_color.'">'.$total_amt_gen.'</td>
...
$over_total = mysqli_query( $connect, "select sum(target), sum(amount_gen), sum(total_amt_gen) from other_inc " .$opt." ")or
...
<td bgcolor="#00FFFF">'.($result_sum[2] - $result_sum[1]).'</td>
</tr>';
