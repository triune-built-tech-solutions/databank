...
<table>
<tr><th colspan="11">View all Users</th></tr>
<tr><th>id</th>
<th>staff N<u>o</u></th>
<th>Name </th>
<th>Gender</th>
...
if(isset($lim)){
	$query_staff_info = "select * from staff_reg where username ='".$lim."' order by id desc";
}else {
	$query_staff_info = "select * from staff_reg order by id desc limit 60";
}
...
for($i=0; $i<mysqli_num_rows($result_staff_info); $i++){
	...
	$access_r = mysqli_result($result_staff_info,  $i,  "access_right");
...
$q_access = "SELECT * FROM assess_right where right_id = $access_r";
$r_access = mysqli_query( $connect, $q_access);
	
while($access_row = mysqli_fetch_array($r_access)){
	$access_show = $access_row[1]; 	
}
...
<td bgcolor="'.$bg_color.'"><a href="reset_right.php?reset='.$id.'">'.$access_show.'</a></td>
</tr>';
}
