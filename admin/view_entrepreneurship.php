...
<table>
<tr><th colspan="12">Entrepreneurship Development Programme</th></tr>
<tr><th>S/n</th>
<th>Office Type</th>
...
if($access_right == 4){
	$query_report_view = "select * from entrepreneurship $opt order by year desc, month desc limit 80";
} else if($access_right == 3 || $access_right == 1){
	$query_report_view = "select * from entrepreneurship where office_location = '".$office_location."' order by year desc, month desc limit 80";
} else {
	$query_report_view = "select * from entrepreneurship where added_by = '".$_SESSION['user_name']."' order by year desc, month desc limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_report_view); $i++){
	...
	$training_title = mysqli_result($result_report_view,  $i,  "training_title");
	$participant = mysqli_result($result_report_view,  $i,  "participant");
...
<td bgcolor="'.$bg_color.'">'.$training_title.'</td>
<td bgcolor="'.$bg_color.'"><a href="view_entre_participants.php?id='.$rep_id.'">'.$participant.'</a></td>
<td bgcolor="'.$bg_color.'">'.$state.'</td>
...
$over_total = mysqli_query( $connect, "select sum(participant) from entrepreneurship " .$opt." ")or
