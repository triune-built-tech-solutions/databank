...
if(isset($opt)){
	$query_part = "select * from nisdp_part $opt WHERE added_by = '$added_by1' order by name limit 80";
} else {
	$query_part = "select * from nisdp_part WHERE added_by = '$added_by1' order by name limit 80";
}
...
for($i=0; $i<mysqli_num_rows($result_part); $i++){
	...
	$train_center = mysqli_result($result_part,  $i,  "training_center");
...
<td bgcolor="'.$bg_color.'">'.$train_center.'</td>
